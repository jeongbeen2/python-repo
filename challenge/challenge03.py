...
def get_from_dict(dict, key=None):
    if type(dict) != str and key != None:
        if key in dict:
            print(key, ':', dict[key])
        elif key not in dict:
            print(f"{key} was not found in this dict.")
    elif type(dict) != str and key == None:
...
def update_word(dict, key, value=None):
    if type(dict) != str and value != None:
        if key in dict:
            dict[key] = value
            print(f"{key} has been updated to:", value)
        elif key not in dict:
            print(f"{key} is not on the dict. Can't update non-existing word.")
    elif type(dict) != str and value == None:
